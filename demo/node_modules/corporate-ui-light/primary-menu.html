...
<dom-module id="primary-menu">
  <template>
    <style>
     ...
     <ul class="nav navbar-nav main-menu" hidden="{{hideMenu}}">
         <template is="dom-repeat" items="{{items}}">
             <li class$="main-menu-item {{conditionalClass('active', item, 'active')}}" >
              <ks-nav-item href="{{item.href}}" title="{{item.title}}" target="{{item.target}}" id="[[index]]"
                           icon-position="right" on-click="onToggleSubmenu">
              </ks-nav-item>
            </li>
          </template>
                ...
                <ul class="dropdown-menu more-dropdown">
                   <template is="dom-repeat" items="{{items}}">
                         <li class$="{{conditionalClass('active', item, 'active')}}">
                              <ks-nav-item href="[[item.href]]" title="[[item.title]]"
                                           target="[[item.target]]" id="[[index]]" on-click="onDropdownItemActive"></ks-nav-item>
                        </li>
                  </template>
              </ul>
        </li>
    </ul>
    <div class="nav navbar-nav" hidden="{{hideMenu}}">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <template is="dom-repeat" items="{{primary.items}}" as="subItem">
                             <li class$="{{conditionalClass('active', subItem, 'active')}}">
                                <ks-nav-item href="[[subItem.href]]" title="[[subItem.title]]" target="[[subItem.target]]"></ks-nav-item>
                             </li>
                        </template>
                    </ul>
                </div>
  ...
  Polymer({
      ...
      _setMoreMenuToActive: function(){
          this.querySelector('.more-trigger').parentNode.classList.add('active');
      },
      doAdapt: function () {
          var moreDropdown = this.querySelector('.more-dropdown'),
                  ...
                  moreActiveItems = moreDropdown.querySelector('.active'),
