...
<dom-module id="ks-main-navigation">
  ...
  <template>
   ...
   <style>
     ...
     :host .navbar-nav .subnav .dropdown-menu > li + li > ::content a:before {
     ...
     :host.sticky-header .main-menu, :host[hidden].sticky-header .main-menu {
       position: fixed;
       top: -1px;
       left: 0;
       right: 0;
     ...
     :host.sticky-header .navbar-nav.navbar-right, :host[hidden].sticky-header .navbar-nav.navbar-right {
       padding-right: 60px;
     }
     ...
     :host.sticky-header ::content .navbar-header .navbar-symbol:before, :host[hidden].sticky-header ::content .navbar-header .navbar-symbol:before {
       position: fixed;
       top: 7px;
       right: 10px;
       transition: 300ms linear, right 300ms linear;
     }
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse main-menu" id="mainMenu">
      <ul class="nav navbar-nav">
        ...
        <li>
          <ks-nav-item href="#2" on-click="onToggleSubmenu" title="Link 2" class="dropdown-toggle" data-toggle="dropdown" id="item-2" role="button"></ks-nav-item>
        </li>
        <li><ks-nav-item href="#3" title="Link 3"></ks-nav-item></li>
        <li>
          <ks-nav-item href="#4" on-click="onToggleSubmenu" title="Link 4" class="dropdown-toggle" data-toggle="dropdown" id="item-4" role="button"></ks-nav-item>
        </li>
        <li><ks-nav-item href="#5" title="Link 5"></ks-nav-item></li>
        <li><ks-nav-item href="#6" title="Link 6"></ks-nav-item></li>
    ...
    <div class="nav navbar-nav" >
      <div  class="dropdown subnav" id="subMenu">
...
<ks-fullscreen hide-elements='["ks-main-footer", "#mainHeader", "#mainMenu"]'></ks-fullscreen>
  ...
  Polymer({
    ...
    onToggleSubmenu: function(event){
      var subMenuClassList = this.$.subMenu.classList,
              isOpen = subMenuClassList.contains('open') && subMenuClassList.contains(event.target.parentNode.id);
      if(isOpen){
        this.$.subMenu.classList.remove('open');
        this.$.subMenu.classList.remove(event.target.parentNode.id);
      }else{
        this.$.subMenu.classList.add('open');
        this.$.subMenu.classList.add(event.target.parentNode.id);
      }
